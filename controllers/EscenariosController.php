...
class EscenariosController {
    ...
    public function registrarReserva() {
        ...
        // Validar datos
        $required_fields = ['escenario_id', 'fecha', 'hora_inicio', 'hora_fin', 'usuario_id'];
        $errors = [];
        ...
        try {
            ...
            $result = $this->escenario_model->registrarReserva($data);

            if ($result) {
                ResponseUtil::sendResponse(null, "Reserva registrada exitosamente");
            } else {
                ResponseUtil::sendError("Error al registrar la reserva", 500);
            }
        } catch (\Exception $e) {
            ResponseUtil::sendError("Error: " . $e->getMessage(), 500);
        }
    ...
    public function obtenerHorasReservadas($escenario_id = null) {
        // Verificar método de solicitud
        if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
            ResponseUtil::sendError("Método no permitido", 405);
            return;
        ...
        // Validar si se proporcionó el ID del escenario
        if (!$escenario_id) {
            ResponseUtil::sendError("ID de escenario no proporcionado", 400);
            return;
        }
        ...
        if (!$fecha) {
            ResponseUtil::sendError("La fecha es requerida", 400);
            return;
        }
    
        try {
            // Obtener las horas reservadas
            $horas_reservadas = $this->escenario_model->obtenerHorasReservadas($escenario_id, $fecha);
    
            ResponseUtil::sendResponse($horas_reservadas, "Horas reservadas obtenidas exitosamente");
        } catch (\Exception $e) {
            ResponseUtil::sendError("Error: " . $e->getMessage(), 500);
        }
    }
