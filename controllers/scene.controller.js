...
export const getAllScenes = async (req, res) => {
  try {
    ...
    if (localidad_id) {
      ...
      queryParams.push(parseInt(localidad_id));
    }
    ...
    if (deporte_id) {
      ...
      queryParams.push(parseInt(deporte_id));
      paramIndex++;
    }
    ...
    const scenesWithDetails = await Promise.all(scenes.map(async (scene) => {
  ...
  try {
    ...
    try {
      ...
      return sendResponse(
        res,
        null,
        'Escenario eliminado exitosamente'
      );
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
